...
The Docling dependency is required to use the Docling components in Langflow.
    ...
    * **Earlier versions**: Langflow versions earlier than 1.6 don't include the Docling dependency.
    For Langflow OSS, install the Docling extra with `uv pip install --trusted-host pypi.org --trusted-host pypi.python.org --trusted-host files.pythonhosted.org 'langflow[docling]'`.
